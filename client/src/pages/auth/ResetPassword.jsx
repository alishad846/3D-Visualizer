...
import { ArrowLeft, Check, View, Eye, EyeOff } from 'lucide-react';
      ...
      <div className="w-full lg:w-1/2 bg-[#1C1D21] flex flex-col items-center justify-center p-8 sm:p-12 lg:p-24 relative">
        ...
        >
          ...
          <p className="text-[#A0A0A0] text-center text-sm mb-8">
            Choose a new password for your ScanVista account.
          </p>
          ...
          {success ? (
            ...
            >
              <div className="w-16 h-16 bg-[#00F0FF]/10 rounded-full flex items-center justify-center mb-4">
                <div className="w-8 h-8 bg-[#00F0FF] rounded-full flex items-center justify-center text-black">
                  <Check className="w-5 h-5" />
                </div>
              </div>
              <h3 className="text-xl font-display font-medium mb-2 text-[#00F0FF]">Password updated</h3>
              <p className="text-[#A0A0A0] text-sm">Taking you to login...</p>
            </motion.div>
          ) : (
            <form onSubmit={handleSubmit} className="space-y-6">
              ...
              {error && (
                <p className="text-red-400 text-xs text-center leading-5">{error}</p>
              )}
          ...
          {!success && (
            <p className="mt-8 text-center text-[13px] text-[#A0A0A0]">
              Need a new link?{' '}
              <Link to="/forgot-password" className="text-white hover:text-[#00F0FF] transition-colors font-medium">Request reset</Link>
            </p>
          )}
        </motion.div>
      </div>
